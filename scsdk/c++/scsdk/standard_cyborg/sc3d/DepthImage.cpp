...
#ifdef PYBIND11_ONLY
DepthImage::DepthImage(int width_, int height_, const NPFloat& depth_) :
width(width_),
height(height_){
    
    SCASSERT(width >= 0, "Width  must be >= 0");
    SCASSERT(height >= 0, "Height  must be >= 0");
    SCASSERT(width * height == depth_.shape(0), "Size of data must match size of image");
    
    auto r = depth_.unchecked<1>();
    for (int i = 0; i < depth_.shape(0); i++) {
        depth.push_back(r(i));
    }
}
#endif // PYBIND11_ONLY
