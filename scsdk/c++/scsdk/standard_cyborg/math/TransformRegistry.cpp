...
Result<bool> TransformRegistry::deregisterTransform(const Transform& transform) {
    if (edges.count(transform.srcFrame) == 0) {
        return {.error = fmt::format(
                                     "Invalid attempt to deregister transform with unknown source frame \"{}\"", transform.srcFrame
                                     )};
    }
    if (edges.count(transform.destFrame) == 0) {
        return {.error = fmt::format(
                                     "Invalid attempt to deregister transform with unknown destination frame \"{}\"", transform.destFrame
                                     )};
    }
    ...
    if (srcEdges.count(transform.destFrame) == 0 || destEdges.count(transform.srcFrame) == 0) {
        return {.error = fmt::format(
                                     "Invalid attempt to deregister transform between disconnected frames: source = \"{}\", destination = \"{}\"",
                                     transform.srcFrame,
                                     transform.destFrame
                                     )};
    }
...
Result<bool> TransformRegistry::registerTransform(const Transform& transform) {
    ...
    if (!areAdjacent) {
        ...
        if (connectedResult.IsOk() && *(connectedResult.value)) {
            return {.error = fmt::format(
                                         "Addition of transforms between frames \"{}\" and \"{}\" would create a circular reference. Ignoring transform.",
                                         transform.srcFrame,
                                         transform.destFrame
                                         )};
        }
...
Result<std::vector<std::string>> TransformRegistry::path(const std::string& srcFrame, const std::string& destFrame) const {
    ...
    if (!edges.count(srcFrame)) return {.error = fmt::format("Invalid access of unknown frame \"{}\"", srcFrame)};
    if (!edges.count(destFrame)) return {.error = fmt::format("Invalid access of unknown frame \"{}\"", destFrame)};
    ...
    while (!frameQueue.empty()) {
        ...
        if (!edges.count(curFrame)) return {.error = fmt::format(
                                                                 "Invalid access of unknown frame \"{}\". This is likely the result of an internal error.",
                                                                 curFrame
                                                                 )};
...
Result<Transform> TransformRegistry::getTransform(const std::string& srcFrame, const std::string& destFrame) const {
    ...
    for (int i = path.size() - 2; i >= 0; i--) {
        if (!edges.count(path[i])) return {.error = fmt::format(
                                                                "Invalid access of frame \"{}\"",
                                                                path[i]
                                                                )};
        const auto adj = edges.at(path[i]);
        
        if (!adj.count(path[i + 1])) return {.error = fmt::format(
                                                                  "Invalid access of frame \"{}\"",
                                                                  path[i + 1]
                                                                  )};
